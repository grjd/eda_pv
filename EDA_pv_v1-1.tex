...
We compute the Fisher's exact test when the contingency table is $2 \times 2$ and the Chi test otherwise \cite{scipy2010}. If the independent variable is defined in the Reals (e.g. weight) we perform a one way ANOVA.
...
Figure \ref{fig:groupby_sexolat} shows the results of grouping by \emph{Conversion to MCI} relative to the sex.

\begin{figure}[H]
        \centering
        \includegraphics[keepaspectratio, width=.8\linewidth]{figures/groupby_sexolat}
        \caption{Frequency of sex and hand laterality related conversion to MCI in the future.} 
        \label{fig:groupby_sexolat}
\end{figure}

The contingency tables displaying the frequency distribution of sex and MCI and hand laterality and MCI are shown in \ref{tab:sexomci} and \ref{tab:handmci} respectively. The significance of the difference between the proportion levels of sex is assessed using the Fisher's exact test and  $\tilde{\chi}^2$ for hand laterality.
...
\begin{minipage}[b]{.40\textwidth}
   \centering
    ...
    \label{tab:sexomci}
  \end{tabular}
   \captionof{table}{Contingency table proportion of conversion to MCI conditioned and sex. The statistical test, $F = 0.416307178710994$.}
   \label{tab:sexomci}
\end{minipage}\qquad
\begin{minipage}[b]{.40\textwidth}
   \centering
    ...
    \label{tab:handmci}
  \end{tabular}
   \captionof{table}{Contingency table proportion of conversion to MCI conditioned and hand laterality. The statistical test, $\tilde{\chi}^2 = 0.05430944$.}
   \label{tab:handmci}
\end{minipage}
...
Figure \ref{fig:groupby_mci_fami}  shows the results of grouping by \emph{Conversion to MCI} relative to familial AD (0: non familial AD and 1: familial AD).

For each chart, on the left the grouping is in absolute numbers and on the right side in relative numbers which is a preferable representation since the dataset is unbalanced (many more non converters to MCI than converters). Note that the colored bars on the right sided chart must sum 1 (a frequentist could interpreted this as a probability function). 
...
\begin{figure}[H]
        ...
        \caption{APO$\epsilon4$ and conversion to MCI in the future, represented as genetic profile conditioned to conversion. Grouping by conversion to MCI relative to the genetic profile of the subjects we obtain $p(MCI|APO\epsilon4)$.} 
...
The contingency table displaying the frequency distribution of APOE and MCI is shown in Table \ref{tab:apoemci}. The significance of the difference between the proportion levels is assessed using the $\tilde{\chi}^2$ test, $\tilde{\chi}^2 = 0.00065$, the imbalance is thus statistically significant and homozygotic APOE subjects have a larger conversion rate to MCI.

\begin{table}[ht]
\centering
\caption{Contingency table showing the proportion of conversion to MCI conditioned to APO$\epsilon$ (lack of allele, heterozygote, homozygote). The statistical test, $\tilde{\chi}^2 = 0.00065$}
...
The contingency table displaying the frequency distribution of familial AD and MCI is shown in Table \ref{tab:famimci}. The significance of the difference between the proportion levels is assessed using the Fisher's exact test, $F = 0.01882$ which means that the probability that we would observe this or an even more imbalanced ratio in familial AD related to MCI by chance is about $2\%$.
%the imbalance is statistically significant for a significance level is $5\%$. 

\begin{table}[ht]
\centering
\caption{Contingency table showing the proportion of conversion to MCI conditioned to familial AD. The Fisher's statistical test, $F = 0.01882$}
\begin{tabular}[t]{lcc}
\hline
...
%Demographics
Figure \ref{fig:groupby_Demosmci} shows the results of grouping by \emph{Conversion to MCI} relative to demographics information such as education level, family size and residence, marital status, self perceived socioeconomic status and work experience. Interestingly, within a frequentist interpretation, the probability to convert to MCI being single is larger than being married which is larger than being widowed which in turn is larger than convert to MCI being a divorcee (See Table \ref{tab:marital} for the specifics). 
...
We do not observe any relevant conditioning of marital status on conversion to MCI.

\begin{table}[ht]
  \centering
  \caption{Contingency table showing the proportion of conversion to MCI and marital status. The statistical test, $\tilde{\chi}^2 = 0.95909$}
  \begin{tabular}[t]{lcccc}
  \hline
...
We do not observe any relevant conditioning of educational level or wealth (residency) on conversion to MCI.
...
\begin{minipage}[b]{.40\textwidth}
   \centering
    ...
    \label{tab:rentamci}
  \end{tabular}
   \captionof{table}{Contingency table proportion of conversion to MCI and wealth. The statistical test, $\tilde{\chi}^2 = 0.3162510$}
   \label{tab:handmci}
\end{minipage}
...
We observe relevant conditioning on conversion to MCI for going to church and also for going to the movie theater.
...
The statistical tests for cardiovascular and cerebrovascular features are included in Table \ref{tab:fischiall}. We observe relevant conditioning on conversion to MCI for xyz


%%%%% Table Loyals  visits %%%%%%%%%
%%%%%  %%%%%%%%%
%%%%%  %%%%%%%%%
Table \ref{tab:fischiall} with statistical tests for both a population of the subjects with 6 visits and also in the second column for subjects that came to at least two visits.

\begin{table}[ht]
\caption{Statistical test for subjects with all their 6 visits and for subjects with at least 2 visits.}
\centering
