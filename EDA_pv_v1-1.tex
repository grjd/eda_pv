...
\begin{figure}[H]
        \centering
        \includegraphics[keepaspectratio, width=.8\linewidth]{figures/groupby_sexolat}
...
\begin{minipage}[b]{.40\textwidth}
   \centering
    ...
    %\label{tab:handmci}
  \end{tabular}
   \captionof{table}{Contingency table: proportion of conversion to MCI conditioned to sex and hand laterality.}
   \label{tab:handmci}
\end{minipage}
...
\begin{figure}[H]
        \centering
        \includegraphics[keepaspectratio, width=.8\linewidth]{figures/groupby_mci_genetics}
...
\begin{figure}[H]
        \centering
        \includegraphics[keepaspectratio, width=.8\linewidth]{figures/groupby_Familial_mciY}
...
\begin{minipage}[b]{.40\textwidth}
   \centering
    ...
    %\label{tab:sexomci}
  \end{tabular}
   \captionof{table}{Contingency table proportion of conversion to MCI and educational level. The statistical test, $\tilde{\chi}^2 = 0.5303475$.}
   \label{tab:edumci}
\end{minipage}\qquad
\begin{minipage}[b]{.40\textwidth}
   \centering
...
Figure \ref{fig:brainviolin} shows the results of the brain extraction algorithm for years 1 and year 6. The brain's size is expressed in mm${^3}$ in native space (left) and scaled in MNI space (right). 
...
\begin{figure}[H]
        ...
        \caption{The violin plot shows the brain volume in native space ($cm^3$) for year 1 (left) and year 6 (right). As expected the mean volume decreases from  year 1 to year 6.} 
        \label{fig:boxbet}
\end{figure}
%\footnote{A violin plot is a hybrid of a box plot and a kernel density plot.}

We calculate brain atrophy as the difference between the estimated volume at year 1 and the estimated volume at year 6. Figure \ref{fig:braingroupviol} shows the atrophy for MCI converters and non MCI converters, grouped by sex and familial AD.
...
\begin{figure}[!htb]
    ...
    \end{minipage}
    \caption{IOn the left, brain atrophy for converters and non converters grouped by familial sex (0:Male, 1:Female) and on the right grouped by familial AD.}
...
Figure \ref{fig:violtissues} shows estimated volume for tissue based segmentation across two time points, year 1 and year 6. The volume distribution of CSF (left), gray matter (middle) and white matter(right) are shown. 
The figure indicates Figure \ref{fig:violtissues} shows an increase in the volume of the CSF and at the same time a decrease in the parenchyma (white and gray matter). 
...
\begin{figure}[H]
        \centering
        \includegraphics[keepaspectratio, width=0.6\linewidth]{figures/tissues_violin}
        \caption{The plot shows the tissue based segmentation results. For each type of tissue we plot the estimated volume in year 1 and year 6. Thus, left csf volume in first and last years, middle gray matter volume in first and last year and right white matter volume for first and last year. } 
        \label{fig:violtissues}
\end{figure}

Figure \ref{fig:jointtissues} shows the relationship between two variables -the atrophy of tissues as well as the profiles of each tissue atrophy in the margins. Thus, on the left the joint plot of gray matter atrophy and white matter atrophy and on the right between gray matter atrophy and CSF atrophy. The figure indicates that there is no linear correlation between the volume atrophy of the brain tissues. 

\begin{figure}[!htb]
    \centering
    \begin{minipage}{.5\textwidth}
        \centering
        \includegraphics[height=0.30\textheight, keepaspectratio]{figures/tissuesgm_joint} %height=0.15\textheight
        %\caption{Brain atrophy for converters and non converters grouped by sex.}
        %\label{fig:braingroupviol-a}
    \end{minipage}%
    \hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[height=0.30\textheight, keepaspectratio]{figures/tissuesgc_joint}
        %\caption{Brain atrophy for converters and non converters grouped by familial AD.}
        %\label{fig:braingroupviol-b}
    \end{minipage}
    \caption{On the left, relationship between the variables gray matter atrophy and white matter; on the right, relationship between the variables gray matter atrophy and CSF atrophy.}
...
\begin{figure}[!htb]
    ...
    \begin{minipage}{.3\textwidth}
        \centering
        \includegraphics[height=0.28\textheight, keepaspectratio]{figures/tissuescsf16_joint} %height=0.15\textheight
        %\caption{Brain atrophy for converters and non converters grouped by sex.}
        %\label{fig:braingroupviol-a}
    \end{minipage}%
    \hfill
    \begin{minipage}{0.3\textwidth}
        \centering
        \includegraphics[height=0.28\textheight, keepaspectratio]{figures/tissuesgm16_joint}
        %\caption{Brain atrophy for converters and non converters grouped by familial AD.}
        %\label{fig:braingroupviol-b}
    \end{minipage}
    \hfill
    \begin{minipage}{0.3\textwidth}
        \centering
        \includegraphics[height=0.28\textheight, keepaspectratio]{figures/tissueswm16_joint}
        %\caption{Brain atrophy for converters and non converters grouped by familial AD.}
        %\label{fig:braingroupviol-b}
    \end{minipage}
...
%so far we present the results for brain extraction for the first year. We plan to show segmentation of subcortical structure for all the years in the short future. Of note, the imaging data has not been stored, let alone preprocessed, with the minimum quality requirements for a project of of such volume and complexity. This is delaying and the work that we intend to show here.
...
\subsection{GroupBy Brain subcortical structures}
...
\label{se:res}
Time series Clustering, characterization and Prediction (and regime shift).
%https://johncarlosbaez.wordpress.com/2015/11/25/regime-shift/
Regime shifts are abrupt substantial changes in complex systems. Phase transition is a easier concept and canonical concept to understand (it does not require a complex system). For regime shift systems do not need to be in thermal equilibrium, while phase transition is how varying the parameters affects the thermal equilibrium. So, whatever a regime is, it’s not a thermodynamic phase. For the case of the brain we are talking about regime shift no phase transitions because it is all about the rupture of feedback loops and no feedback loops are conceived in the phase transition picture, just a balance of forces.
%Planet Earth experienced a global climate shift in the late 1980s on an unprecedented scale, fueled by anthropogenic warming and a volcanic eruption, according to new research.

Baez lit. Phase transition means something pretty specific: you have system whose thermal equilibrium state depends on a some parameters, and as you adjust the parameters, observable quantities vary smoothly except at certain points in the parameter space, which usually lying on hypersurfaces called ‘phase transitions’. The complement of these surfaces has connected components called ‘phases’. There are also more specific concepts like ‘first-order phase transition’ and ‘second-order phase transition’.
...
Wiki lit.In ecology, regime shifts are large, abrupt, persistent changes in the structure and function of a system. A regime is a characteristic behavior of a system which is maintained by mutually reinforced processes or feedbacks.The change of regimes, or the shift, usually occurs when a smooth change in an internal process (feedback) or a single disturbance (external shocks) triggers a completely different system behavior. Crucially, the concept of ‘feedback’ is largely absent from the concept of thermodynamic phase, because feedback typically involves flow around loops, while in thermal equilibrium the principle of detailed balance prohibits flow around loops.

%https://www.beringclimate.noaa.gov/regimes/
Methodology to look for trends: The STARS method (Rodionov, 2004; Rodionov  Overland, 2005) tests whether the end of one period (regime) of a certain length is different from a subsequent period (new regime). The cumulative sum of normalized deviations from the hypothetical mean level of the new regime is calculated, and then compared with the mean level of the preceding regime. A shift year is detected if the difference in the mean levels is statistically significant according to a Student’s t-test.

%https://www.youtube.com/watch?v=MP-jMMOgdjQ
ML of time series:

-Minimum bias optimization (convex optimization). To produce a complete graph relate all securities to each other we need to calculate N(N+1)/2 covariance estimations, will not be robust, because unless you have stable correlations for the past eg 5 years, your covariance matrix (day by day) is going to change very widely. For example, my covariance matrix for the previous 6 years, maybe for the next seventh year is going to be very different,. YS: check this out.
Problems to invert the cov matrix: The inverse covariance matrix, commonly referred to as the precision matrix displays information about the partial correlations of variables. A partial correlation describes the correlation between variable i and j, once you condition on all other variables. 
ML allows to go around the cov matrix inverse problem exploiting the hierarchical relationships between the securities, as a consequence the estimate are more robust because rather than having to estimate N(N+1)/2 we just need N-1 estimates.

-Structural Breaks

ML models are particularly good at detecting change: prob. that the observation belongs to past observed clusters, the goal here is to derive a prob of shit and not a point prediction. Thus, probability of regime shift (new obs is not in the dictionary) rather than predict a price. (YS: for this surely will need long time series which i do not have for this study). A ML methodology for regime shift is SADF; Does the series exhibit bubble-like patterns? if this is the case mean reverting models must be avoided. The idea is not to predict when the bubble will pop but that there is a bubble in formation.

-Bet sizing

Precision - Recall
One algorithm with High P and low R and then a second algo that does the opposite so you get a good F1 score, the second algo is not saying to buy or sell but to tell you if you should go or not for the bet.
Which are the truly relevant variables? to predict X? for example tell to newton that what matter for the force of attraction between two bodies are mass and distance not the shape or color. The ML will never gives us a closed form solution, but nevertheless it assist by telling us what are the variables (input) that are relevant.
In a high dimensional space, the relationship between features can be extremely complex, however ML can tell us which are the ones that matter.


\subsection{The \emph{Buschke Integral} score}
